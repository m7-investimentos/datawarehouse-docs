...
class PerformanceAssignmentsETL:
    ...
    def setup_connections(self):
        ...
        try:
            creds_path = self.config.get(
                "google_credentials_path", CREDENTIALS_DIR / "google_sheets_api.json"
            )
        ...
        try:
            with self.db_engine.connect() as conn:
                ...
                self.indicators_df = pd.DataFrame(
                    result.fetchall(), columns=["indicator_code"]
                )
                self.logger.info(
                    f"Carregados {len(self.indicators_df)} indicadores para validação"
                )
        except Exception as e:
    ...
    def extract(self) -> pd.DataFrame:
        ...
        try:
            ...
            if len(values) < 10:
                raise ValueError(
                    f"Poucos dados na planilha: {len(values)} linhas (mínimo: 10)"
                )
            ...
            for row_idx, row in enumerate(data):
                ...
                if not row or (row and not row[0].strip()):
                    ...
                    if empty_row_count >= 5:
                        self.logger.info(
                            f"Encontradas {empty_row_count} linhas vazias consecutivas, parando leitura"
                        )
                        break
                    continue
                else:
            ...
            self.logger.info(
                f"Extraídos {len(self.data)} registros válidos de atribuições"
            )
    ...
    def standardize_assignments(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["indicator_code"] = (
            df["indicator_code"].str.upper().str.replace(" ", "_").str.strip()
        )
    ...
    def validate_weights(self, df: pd.DataFrame) -> List[Dict]:
        ...
        weight_sums = card_df.groupby(["codigo_assessor_crm", "valid_from"])[
            "weight"
        ].sum()
    ...
    def validate_relationships(
        self, df: pd.DataFrame, indicators_df: pd.DataFrame
    ) -> List[Dict]:
        """
    ...
    def validate_data(self) -> bool:
        ...
        for idx, row in self.data.iterrows():
            ...
            if (
                ...
                and row["indicator_type"] not in VALID_INDICATOR_TYPES
            ):
                self.validation_errors.append(
                    {
        ...
        critical_errors = [
            ...
            if e["error_type"] in ["MISSING_REQUIRED_FIELD", "EMPTY_REQUIRED_FIELD"]
        ]

        if critical_errors:
            self.logger.error(
                f"Encontrados {len(critical_errors)} erros críticos de validação"
            )
        ...
        self.logger.info(
            "Dados validados com sucesso (erros não críticos serão registrados)"
        )
        return True
    ...
    def transform(self) -> pd.DataFrame:
        ...
        if weight_errors:
            ...
            self.logger.warning(
                f"{len(weight_errors)} erros de validação de peso encontrados"
            )
        ...
        df["row_hash"] = df[hash_columns].apply(
            lambda x: hashlib.md5(
                "_".join(str(x[col]) for col in hash_columns).encode()
            ).hexdigest(),
            axis=1,
        )
        ...
        for error in self.validation_errors:
            if error["error_type"] == "INVALID_WEIGHT_SUM":
                mask = (
                    (df["codigo_assessor_crm"] == error["codigo_assessor_crm"])
            ...
            try:
                ...
                if self.validation_errors:
                    ...
                    load_data["validation_errors"] = load_data.apply(
                        lambda x: (
                            json.dumps(
                                error_dict.get(
                                    f"{x['codigo_assessor_crm']}_{x['indicator_code']}_{x['valid_from']}",
                                    [],
                                )
                            )
                            if f"{x['codigo_assessor_crm']}_{x['indicator_code']}_{x['valid_from']}"
                            in error_dict
                            else None
                        ),
                        axis=1,
                ...
                for i in range(0, total_rows, batch_size):
                    ...
                    try:
                        ...
                        self.logger.debug(
                            f"Inseridos registros {i+1} a {min(i+batch_size, total_rows)}"
                        )
                    except Exception as batch_error:
                        self.logger.error(
                            f"Erro ao inserir lote {i//batch_size + 1}: {batch_error}"
                        )
                        ...
                        for idx, row in batch.iterrows():
                            try:
                                ...
                                self.logger.error(
                                    f"Erro ao inserir linha {idx}: {row_error}"
                                )
                ...
                or json.dumps(
                    {
                        ...
                        "weight_errors": len(
                            [
                                e
                                for e in self.validation_errors
                                if e["error_type"] == "INVALID_WEIGHT_SUM"
                            ]
                        ),
                        "relationship_errors": len(
                            [
                                e
                                for e in self.validation_errors
                                if e["error_type"] == "INVALID_INDICATOR_CODE"
                            ]
                        ),
                    }
                ),
    ...
    def post_load_validations(self):
        ...
        try:
            with self.db_engine.connect() as conn:
                ...
                self.logger.info(
                    f"Assessores únicos carregados: {result.assessores_unicos}"
                )
                ...
                if result.assessores_com_erro > 0:
                    self.logger.warning(
                        f"Assessores com soma de pesos inválida: {result.assessores_afetados}"
                    )

                # Notificar se houver muitos erros
                if len(self.validation_errors) > 50:
                    self.logger.warning(
                        "Alto número de erros de validação - verificar dados de origem"
                    )
    ...
    def run(self, dry_run: bool = False):
        ...
        try:
            ...
            if self.validation_errors:
                ...
                self.logger.warning(
                    f"Total de erros de validação: {len(self.validation_errors)}"
                )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Executa sem carregar dados no banco"
    )

    parser.add_argument(
        "--validate-only", action="store_true", help="Apenas valida dados sem carregar"
    )
