-- ==============================================================================
-- PROCEDURE CORRIGIDA - bronze.prc_bronze_to_silver_assignments
-- ==============================================================================
-- Correções aplicadas:
-- 1. Linha 263: Corrigido COUNT(DISTINCT) com valid_from
-- 2. Linha 226-228: Adicionado alias na query de debug
-- 3. Linha 274-283: Adicionado alias nas queries de debug
-- ==============================================================================
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            BEGIN
                ...
                -- CORREÇÃO: Adicionado alias na tabela temporária
                SELECT DISTINCT s.indicator_code
                FROM #assignments_staging s
                WHERE s.indicator_exists = 0;
            END
        END
        ...
        BEGIN
            ...
            -- CORREÇÃO: Contar erros de validação usando subquery
            ...
            BEGIN
                ...
                SELECT DISTINCT 
                    s.crm_id,
                    s.valid_from,
                    SUM(s.indicator_weight) as soma_atual,
                    100.00 as soma_esperada
                FROM #assignments_staging s
                WHERE s.indicator_type = 'CARD'
                  AND s.weight_sum_valid = 0
                GROUP BY s.crm_id, s.valid_from;
            END
        END
        ...
        UPDATE m
        SET 
            m.valid_to = DATEADD(DAY, -1, s.new_valid_from),
...
PRINT '1. Adicionado alias "s" nas queries de debug (linhas 226-228 e 274-283)';
PRINT '2. Mantida correção do COUNT(DISTINCT) na linha 263';
PRINT '3. Todos os acessos a colunas de tabelas temporárias agora usam alias adequado';