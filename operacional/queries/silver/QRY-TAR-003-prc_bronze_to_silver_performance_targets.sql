...
BEGIN
    ...
    BEGIN TRY
            ...
            BEGIN
                ...
                SET processing_notes = ISNULL(processing_notes, '') + 
                    ...
                    (SELECT missing_months FROM #completeness_check c 
                     WHERE c.codigo_assessor_crm = bronze.performance_targets.codigo_assessor_crm 
        ...
        SELECT 
            UPPER(LTRIM(RTRIM(t.codigo_assessor_crm))) as codigo_assessor_crm,
            i.indicator_id,
        ...
        USING valid_targets AS source
            ON target.codigo_assessor_crm = source.codigo_assessor_crm
        ...
        WHEN NOT MATCHED BY TARGET THEN
            INSERT (
                codigo_assessor_crm, indicator_id, period_type, period_start, period_end,
                target_value, stretch_target, minimum_target,
                is_active, created_date, created_by,
                source_system, bronze_load_id
            )
            VALUES (
                source.codigo_assessor_crm, source.indicator_id, source.period_type,
        ...
        BEGIN
            ...
            SELECT 
                i.indicator_code,
                i.indicator_name,
                COUNT(DISTINCT t.codigo_assessor_crm) as assessores,
